...
def test_command_refiner_returns_command_output(tmp_path, monkeypatch: MonkeyPatch) -> None:
    variant_path = tmp_path / "variant.txt"
    script = tmp_path / "copy_refiner.py"
    script.write_text(
        "from pathlib import Path\n"
        "import sys\n"
        "Path(sys.argv[3]).write_text(sys.argv[4], encoding='ascii')\n"
    ...
    monkeypatch.setenv(
        "DIFIX3D_COMMAND",
        f"{sys.executable} {script} {{input}} {{output}} {variant_path} {{variant}}",
    )
    monkeypatch.delenv("DIFIX3D_WORKER_COMMAND", raising=False)
    monkeypatch.delenv("DIFIX3D_PYTHON_CALLABLE", raising=False)
    ...
    assert variant_path.read_text(encoding="ascii") == "difix3d_plus"


def test_legacy_backend_value_still_sets_variant_label(
    tmp_path,
    ...
    monkeypatch.delenv("DIFIX3D_VARIANT", raising=False)
    monkeypatch.setenv(
        "DIFIX3D_COMMAND",
        f"{sys.executable} {script} {{input}} {{output}} {variant_path} {{variant}}",
    )
...
def test_command_refiner_runtime_error_preserves_fallback(
    ...
    monkeypatch.setenv("REFINER_BACKEND", "adapter")
    monkeypatch.setenv("DIFIX3D_VARIANT", "difix3d")
    monkeypatch.setenv(
        "DIFIX3D_COMMAND",
